...
function AddBookingForm(props) {
    ...
    const handleAddStripeConnection = async () => {
        try {
            ...
            else if (error.response.data === "You already have a Stripe connection") {
                setStripeConnected(false);
    ...
    const renderBookingsForm = () => {
        ...
        const handleBookingInfoSave = async (e) => {
            ...
            if (!formInput.bookingInfo ){
                props.errorCallback("Please enter some information about your services before saving");
                return;
            }

            
            try {
                const resp = await UserService.setStripeConnection(props.userId, formInput.bookingInfo, formInput.allowBookings);
                if (resp.status === 200){
        ...
        const handleBookingInfoCancel = async (e) => {
            e.preventDefault();
            if (!formInput.bookingInfo ){
                props.errorCallback("Please enter some information about your services before leaving");
                return;
            }
        ...
        return (
            <div className={style.addModule_container}>
                <div className={module_form_class}>
                    ...
                    <button className={style.module_form_button + " " + style.save} type="button"
                            onClick={handleAddStripeConnection}>Add
                    </button>
                </div>
            </div>
